...
// MAP: Massive size, smaller tile = more world visible = infinite feel
const TILE = 22, MAP_W = 200, MAP_H = 150;
...
function initGame() {
  game={
    ...
    waveTimer:0,waveDelay:180,betweenWaves:false,
    bossActive:false,boss:null,
    arenaMode:false,arenaRoom:null,arenaEnemiesLeft:0,
    map:[],rooms:[],arenas:[],floorTiles:[],
...
function generateMap() {
  ...
  // MASSIVE map — many more rooms, bigger rooms
  for (let i=0;i<350;i++) {
    const rw=6+~~(Math.random()*14), rh=5+~~(Math.random()*10);
  ...
  for (let i=0;i<arenaCount;i++) {
    const aw=12+~~(Math.random()*8), ah=10+~~(Math.random()*6);
    const ax=3+~~(Math.random()*(MAP_W-aw-6)), ay=3+~~(Math.random()*(MAP_H-ah-6));
    ...
    const nr=rooms.reduce((b,r)=>{const d=Math.hypot(~~(ax+aw/2)-r.cx,~~(ay+ah/2)-r.cy);return d<b.d?{r,d}:b;},{r:rooms[0],d:Infinity}).r;
  ...
  rooms.forEach(r=>{
    if(r.w>=10&&r.h>=8&&Math.random()<.4) {
      map[r.y+2][r.x+2]=1; map[r.y+2][r.x+r.w-3]=1;
      map[r.y+r.h-3][r.x+2]=1; map[r.y+r.h-3][r.x+r.w-3]=1;
    }
  });

  // Build floorTiles list — ONLY these positions are valid for spawning
  const floorTiles = [];
  for(let y=0;y<MAP_H;y++)
    for(let x=0;x<MAP_W;x++)
      if(map[y][x]===0) floorTiles.push({x,y});

  return {map,rooms,arenas,floorTiles};
}
...
//  ENEMY CREATION + AI
// ═══════════════════════════════════════
// Safe spawn — GUARANTEED floor tile, never in wall, away from player
function safeSpawnPos(minDistFromPlayer=250) {
  const p = game.player;
  const ft = game.floorTiles;
  if(!ft||ft.length===0) return {x:TILE*10,y:TILE*10};

  // Shuffle a sample and find first valid tile
  const sampleSize = Math.min(ft.length, 200);
  const start = ~~(Math.random()*(ft.length-sampleSize));
  for(let i=0;i<sampleSize;i++){
    const t = ft[(start+i) % ft.length];
    const wx = (t.x+.5)*TILE, wy = (t.y+.5)*TILE;
    if(Math.hypot(wx-p.x, wy-p.y) >= minDistFromPlayer) return {x:wx, y:wy};
  }
  // Fallback: any floor tile far enough, even if close
  const t = ft[~~(Math.random()*ft.length)];
  return {x:(t.x+.5)*TILE, y:(t.y+.5)*TILE};
}
...
function mkEnemy(typeId, wave, spawnX, spawnY) {
  ...
  if(spawnX!==undefined && spawnY!==undefined){
    // Validate: make sure given pos is on floor
    if(tAt(spawnX,spawnY)===0){ sx=spawnX; sy=spawnY; }
    else { const sp=safeSpawnPos(100); sx=sp.x; sy=sp.y; }
  } else {
    const sp=safeSpawnPos(280); sx=sp.x; sy=sp.y;
  }
  return {
    ...def,
    x:sx,y:sy,vx:0,vy:0,
    ...
    alertRange: def.elite ? 380 : 260,
    attackRange: def.fire>0 ? 280 : 40,
    retreatHp: def.hp * .25,
...
// ═══════════════════════════════════════
//  HORDE MODE — constant spawning
// ═══════════════════════════════════════
const HORDE_CAP      = 60;  // max enemies on screen at once
const HORDE_INTERVAL = 45;  // ticks between spawn attempts (~0.75s at 60fps)

function getHordeTypes(wave) {
  let types=[0,0,0,0,3,3]; // mostly chasers early
  if(wave>=2) types.push(3,3,0,0);
  if(wave>=3) types.push(1,1,0);
  if(wave>=4) types.push(5,3,1);
  if(wave>=5) types.push(2,0,3,1);
  if(wave>=6) types.push(6,1,3);
  if(wave>=7) types.push(4,1,2);
  if(wave>=8) types.push(7,2,6);
  if(wave>=3&&wave%3===0) types.push(8,9,8);
  if(wave>=6&&wave%6===0) types.push(10,11,12,9);
  return types;
}

function hordeSpawn() {
  if(!game.running||game.paused||game.betweenWaves) return;
  if(game.bossActive) return;
  const alive=game.enemies.filter(e=>e.alive).length;
  if(alive>=HORDE_CAP) return;
  // Spawn 2-6 enemies per interval based on wave
  const toSpawn=Math.min(HORDE_CAP-alive, 2+~~(game.wave/2));
  const types=getHordeTypes(game.wave);
...
// ═══════════════════════════════════════
//  WAVE SYSTEM (horde — time-based)
// ═══════════════════════════════════════
const WAVE_DURATION = 60 * 35; // 35 seconds per wave
...
function startWave() {
  ...
  // Boss wave
  if(game.wave%5===0){ showBossWarn(); return; }

  // Spawn initial burst — big enough to feel like a horde
  const initCnt=Math.min(12+game.wave*3, HORDE_CAP);
  const types=getHordeTypes(game.wave);
  ...
  assignGroups();
  setStatus(`[ WAVE ${game.wave} INITIATED — HORDE INCOMING ]`,150);
  checkArenaTrigger();
}
...
function startArena(arena) {
  ...
  // Build arena-specific floor tiles
  const arenaFloor=[];
  for(let ty=arena.y;ty<arena.y+arena.h;ty++)
    for(let tx=arena.x;tx<arena.x+arena.w;tx++)
      if(game.map[ty]?.[tx]===0) arenaFloor.push({x:tx,y:ty});

  const cnt=6+game.wave;
  const eliteTypes=[8,9,10,11,12];
  for(let i=0;i<cnt;i++) {
    const t=eliteTypes[~~(Math.random()*eliteTypes.length)];
    // Pick a random floor tile inside arena, not on player
    let tile=arenaFloor[~~(Math.random()*arenaFloor.length)];
    // Avoid spawning right on top of player
    for(let attempt=0;attempt<20;attempt++){
      const candidate=arenaFloor[~~(Math.random()*arenaFloor.length)];
      if(Math.hypot(candidate.x*TILE-game.player.x,candidate.y*TILE-game.player.y)>80){tile=candidate;break;}
    }
    const sx=(tile.x+.5)*TILE, sy=(tile.y+.5)*TILE;
...
function update() {
  ...
  // Horde spawning every HORDE_INTERVAL ticks
  if(game.t%HORDE_INTERVAL===0 && !game.bossActive && !game.arenaMode) hordeSpawn();

  // Wave advances on timer
  if(!game.bossActive&&!game.arenaMode){
    game.waveTimer++;
    if(game.waveTimer>=WAVE_DURATION) endWave();
  }
...
function updateEnemies() {
  ...
  if(game.enemies.length===0&&!game.betweenWaves&&!game.bossActive&&!game.arenaMode) {
    // In horde mode enemies always respawn, but if somehow all die give bonus
    game.score+=500;
  }
}
...
function endWave() {
  ...
  game.waveTimer=0;
  // Keep enemies alive — horde never fully clears
  setStatus(`[ WAVE ${game.wave-1} SURVIVED — NEXT WAVE IN 3s ]`,180);
...
function updateHUD() {
  ...
  // Show wave + seconds countdown
  if(!game.betweenWaves&&!game.bossActive){
    const secsLeft=Math.max(0,~~((WAVE_DURATION-game.waveTimer)/60));
    document.getElementById('wave-val').textContent=`${String(game.wave).padStart(2,'0')} ${secsLeft}s`;
  } else {
...
function startGame() {
  ...
  const {map,rooms,arenas,floorTiles}=generateMap();
  game.map=map; game.rooms=rooms; game.arenas=arenas; game.floorTiles=floorTiles;
  game.setWep=setWep;
